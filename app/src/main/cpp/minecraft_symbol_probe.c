...
static jstring probe_run(JNIEnv* env, const char* requested_directory) {
    ...
    for (int index = 0; index < symbol_count; ++index) {
        ...
        Dl_info info;
        memset(&info, 0, sizeof(info));
        bool owned = dladdr(symbol, &info) != 0 && info.dli_fbase != NULL;
        uintptr_t offset = owned
        ...
        snprintf(line, sizeof(line),
                ...
                "  invoked=NO\n",
                name,
                owned && info.dli_fname != NULL ? info.dli_fname : "UNKNOWN",
    ...
    if (close_failures != 0) {
        ...
        verdict = "CANONICAL ENTRYPOINTS HIDDEN; ELF EXPORT INVENTORY REQUIRED";
    }
    ...
    snprintf(summary, sizeof(summary),
            ...
            "Symbols found: %d\n"
            "JNI_OnLoad found: %s\n"
            "ANativeActivity_onCreate found: %s\n"
            "android_main found: %s\n"
            "Explicit symbols invoked: NO\n"
            "Close failures: %d\n"
            "Probe verdict: %s",
            symbol_count,
    ...
    __android_log_print(
            ANDROID_LOG_INFO,
            PROBE_LOG_TAG,
            "Stage 3.6 finished: found=%d closeFailures=%d",
            found_count,
            close_failures);
